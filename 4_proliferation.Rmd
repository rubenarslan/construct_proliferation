...
inext_all <- iNEXT(counts_all_wide %>% tibble::column_to_rownames("Test"), q = 0, datatype = "abundance")
inext_lumped <- iNEXT(counts_lumped_wide %>% tibble::column_to_rownames("Test"), q = c(0,1, 2), datatype = "abundance")
inext_constructs <- iNEXT(counts_construct_wide %>% tibble::column_to_rownames("Test"), 0, datatype = "abundance")
# inext_all <- iNEXT(abundance_list_all, q = 0, datatype = "abundance")


### 5. Plot the Coverage-Based Rarefaction/Extrapolation Curves
ggiNEXT(inext_all, type = 1) +
  labs(title = "Coverage-Based Diversity Estimates (All Tests)",
       subtitle = "q = 0 (Species Richness); x-axis is sample coverage",
       x = "Sample Coverage",
       y = "Diversity")

ggiNEXT(inext_lumped, type = 1) +
  labs(title = "Coverage-Based Diversity Estimates (Lumped Tests)",
       subtitle = "q = 0 (Species Richness); x-axis is sample coverage",
       x = "Sample Coverage",
       y = "Diversity")

ggiNEXT(inext_constructs, type = 1) +
  labs(title = "Coverage-Based Diversity Estimates (Constructs)",
       subtitle = "q = 0 (Species Richness); x-axis is sample coverage",
       x = "Sample Coverage",
       y = "Diversity")
...
ggplot(inext_combined, aes(x = Year, y = Estimator, ymin = LCL, ymax = UCL, group = type, color = type)) +
  ...
  labs(title = "Coverage-Standardized Diversity Metrics Across Years",
       subtitle = "Columns: Count type; Rows: Diversity metrics (Richness, Shannon, Simpson, True Diversity)",
       x = "Year",
       y = "Diversity Value") +
...
# Plot the asymptotic estimates
ggplot(asy_est_all_sub, aes(x = as.numeric(Assemblage), y = Estimator, color = subdiscipline)) +
  geom_smooth(stat = "identity", size = 0.9) +
  scale_y_continuous("Asymptotic Hill–Shannon Diversity") +
  scale_x_continuous("Year", limits = c(1993, 2030)) +
  theme_minimal(base_size = 14) +
  labs(title = "Asymptotic Hill–Shannon Diversity (All Measures)",
       subtitle = "Estimates by subdiscipline using iNEXT")
ggsave("figures/asy_entropy_subdiscipline_all.png", width = 8, height = 4)
...
ggplot(asy_est_orig_sub, aes(x = as.numeric(Assemblage), y = Estimator, color = subdiscipline)) +
  geom_smooth(stat = "identity", size = 0.9) +
  scale_y_continuous("Asymptotic Hill–Shannon Diversity") +
  scale_x_continuous("Year", limits = c(1993, 2030)) +
  theme_minimal(base_size = 14) +
  labs(title = "Asymptotic Hill–Shannon Diversity (Original Measures)",
       subtitle = "Estimates by subdiscipline using iNEXT")
ggsave("figures/asy_entropy_subdiscipline_orig.png", width = 8, height = 4)
...
ggplot(asy_est_constructs_sub, aes(x = as.numeric(Assemblage), y = Estimator, color = subdiscipline)) +
  geom_smooth(stat = "identity", size = 0.9) +
  scale_y_continuous("Asymptotic Hill–Shannon Diversity") +
  scale_x_continuous("Year", limits = c(1993, 2038)) +
  theme_minimal(base_size = 14) +
  labs(title = "Asymptotic Hill–Shannon Diversity (Constructs)",
       subtitle = "Estimates by subdiscipline using iNEXT")
ggsave("figures/asy_entropy_subdiscipline_constructs.png", width = 8, height = 4)
```
...
ggplot(rare_long, aes(x = Year, y = mean, min=lower, ymax = upper)) +
  geom_smooth(stat = "identity") +
  # geom_line(stat = 'summary', fun = mean, color = "blue", size = 0.7, group = 1) +
  facet_grid(metric ~ type, scales = "free_y") +
  theme_minimal(base_size = 14) +
  labs(title = "Rarefaction Diversity Metrics Across Years",
       subtitle = "Columns: Count type; Rows: Diversity metrics",
       x = "Year",
       y = "Metric value")
```
...
```{r}
ggplot(rare_long %>% filter(metric %in% c("richness", "hill_shannon", "hill_simpson")), aes(x = Year, y = mean, min=lower, ymax = upper, color = type)) +
  geom_smooth(stat = "identity") +
  # geom_line(stat = 'summary', fun = mean, color = "blue", size = 0.7, group = 1) +
  facet_grid(metric ~ ., scales = "free_y") +
  theme_minimal(base_size = 14) +
  labs(title = "Rarefaction Diversity Metrics Across Years",
       subtitle = "Columns: Count type; Rows: Diversity metrics (richness, Shannon, Gini-Simpson)",
       x = "Year",
       y = "Metric value") +
  scale_color_brewer(palette = "Set1")
```
...
```{r}
ggplot(rare_long %>% filter(metric %in% c( "hill_shannon")), aes(x = Year, y = mean, min=lower, ymax = upper, color = type)) +
  geom_smooth(stat = "identity") +
  theme_minimal(base_size = 14) +
  labs(title = "Rarefaction Diversity Metrics Across Years",
       subtitle = "Columns: Count type; Rows: Diversity metrics (richness, Shannon, Gini-Simpson)",
...
rarefaction_hill_shannon <- rare_long %>% filter(metric %in% c( "hill_shannon")) %>% 
  mutate(type = case_match(type, 
                           "All tests" ~ "measures",
...
### 5. Create a facet plot of the results
ggplot(rare_long, aes(x = Year, y = mean, ymin = lower, ymax = upper)) +
  geom_smooth(stat = "identity") +
  facet_grid(metric ~ type, scales = "free_y") +
  theme_minimal(base_size = 14) +
  labs(title = "Rarefaction Diversity Metrics Across Years (Error Added Each Sample)",
       subtitle = "For published tests, counts are adjusted by ±1 with 25% probability anew per simulation",
       x = "Year",
       y = "Metric value")

ggplot(rare_long %>% filter(metric %in% c("richness", "true_diversity")), aes(x = Year, y = mean, min=lower, ymax = upper, color = type)) +
  geom_smooth(stat = "identity") +
  # geom_line(stat = 'summary', fun = mean, color = "blue", size = 0.7, group = 1) +
  facet_grid(metric ~ ., scales = "free_y") +
  theme_minimal(base_size = 14) +
  labs(title = "Rarefaction Diversity Metrics Across Years + 25% chance off by one",
       subtitle = "Columns: Count type; Rows: Diversity metrics (richness, Shannon, Gini-Simpson)",
       x = "Year",
       y = "Metric value") +
  scale_y_log10()+
...
ggiNEXT(entropy_by_class_inext)
